...
# range for likert scale d$Game1Angry1

unique (d$Game1Angry1)
hist(d$Game1Angry1)

# Both commands indicate that the likert scale ranges from 1 to 7
...
performance_diff_summary_d = performance_diff_d %>%
  ...
  summarize(MeanImprovementScore = mean(ImprovementScore, na.rm = T),
            n = n(),
            SD = sd(ImprovementScore, na.rm = T),
            SE = SD/sqrt(n)
            )
...
ggplot(performance_diff_summary_d, 
       aes(x=ConfrontationalGame, y=MeanImprovementScore, 
           ymin=MeanImprovementScore-SE, ymax=MeanImprovementScore+SE, 
           fill=MusicCondition)) +
  geom_bar(position="dodge", stat="identity") + 
  scale_fill_brewer(palette="Set1") +
